...
{
    return(((sizeof(double)==sizeof(real_T)) ? DBL_EPSILON:FLT_EPSILON)*t*16);
}
...
{   
    ...
    if ((pmat=matOpen(inportFileName,"rb")) == NULL) {
        ...
        *matFileDataFormat = MULTIPLEVAR_LIST;
        ...
        for (inportIdx = 0; inportIdx < gblNumRootInportBlks; ++inportIdx){
            ...
            if (!mxIsStruct(mxInputPtr)) {
                ...
                if (mxIsEmpty(mxInputPtr)) continue;
            ...
            if (!mxIsStruct(mxInputPtr)) {
                ...
                if (numOfTimePoints > 0) {
                    ...
                    if (gblInportDataTypeIdx[inportIdx] != SS_FCN_CALL) {
                        ...
                        timeDataIdx= 0;
                        /* Mallocate the arrays for time and data based on the
...
int_T rt_isTimeHit(real_T t, real_T timePoint) {
    if ((t >= timePoint) &&                /* right sided only */
        ((t - timePoint) <= rapid_eps(t))) /* test for epsilon */
        return true;
    else 
        return false;
}
