 ...
 *
 * File: ext_serial_pkt.c
 *
 * Abstract:
 *  The External Mode Serial Packet is a container object for the serial data
 *  to be transmitted or received and the serial communication overhead (such as
 *  headers, trailers, etc.).  The external mode code writes data to be
...
PUBLIC boolean_T GetExtSerialPacket(ExtSerialPacket *pkt, ExtSerialPort *portDev)
{
    ...
    for(;;) {
        ...
        error = ExtSerialPortGetData(portDev, &char1, 1, &numCharRecvd);    
            ...
            if (pkt->inQuote) {
                ...
                switch (char1) {
                  case escape_character:
                    pkt->inQuote = true;
                    /* Need to go get next character at this point. */
                    continue;
                    break;
                    /*
